...
const BiomechanicsBrowser = () => {

    const viewerRef = useRef();
    const containerRef = useRef();
    const guiRef = useRef();

    const { fetchBiomechanicsTrials, fetchBiomechanics } = useContext(AcquisitionState);
    ...
    useEffect(() => {
        fetchBiomechanicsTrials().then((data) => {

            console.log("Biomechanical recordings: ", data);
    ...
    useEffect(() => {
        ...
        if (selectedVideo != null && selectedVideo.length > 0) {
            fetchBiomechanics(selectedVideo).then((data) => {
    ...
    const openViewer = (biomechanics) => {

        console.log("Mesh data: ", biomechanics)
        system.biomechanics = biomechanics;
...
export default BiomechanicsBrowser;