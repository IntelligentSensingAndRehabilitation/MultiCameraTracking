...
def scale_model(
    ...
    point_names,
    output_path="/tmp/",
    model_name="LaiArnoldModified2017_poly_withArms_weldHand",
    markerset_file="RajagopalModified2016_markers_mmpose.xml",
):

    # get path to this file
    path = os.path.dirname(os.path.abspath(__file__))
    model_path = os.path.join(path, "OpenCapOpenSimPipeline", "Models")
    generic_model_path = os.path.join(model_path, model_name + ".osim")
    markerset_path = os.path.join(model_path, markerset_file)

    opensim.Logger.setLevelString("error")
    genericModel = opensim.Model(generic_model_path)
    markerSet = opensim.MarkerSet(markerset_path)
...
def runScaleTool(
    ...
    pathOutputSetup = os.path.join(pathOutputFolder, "Setup_Scale_" + scaledModelName + ".xml")
    pathUpdGenericModel = os.path.join(pathOutputFolder, scaledModelNameA[:-5] + "_generic.osim")
    ...
    for task in markerPlacer.getIKTaskSet():
        ...
        if task.getName() not in coordNames and task.getConcreteClassName() == "IKCoordinateTask":
            task.setApply(False)
            print("{} is a locked coordinate - ignoring IK task".format(task.getName()))
        # Remove Marker tracking tasks for markers not in model.
        if task.getName() not in modelMarkerNames and task.getConcreteClassName() == "IKMarkerTask":
            task.setApply(False)
...
# %% Inverse kinematics.
def runIKTool(
    pathGenericSetupFile, pathScaledModel, pathTRCFile, pathOutputFolder, timeRange=[], IKFileName="not_specified"
):
...
def runIDTool(
    ...
    pathOutputSetupEL = os.path.join(pathOutputFolder, "Setup_EL_" + IKFileName + ".xml")
    pathOutputSetupID = os.path.join(pathOutputFolder, "Setup_ID_" + IKFileName + ".xml")
...
def addOpenPoseMarkersTool(
    pathModel, adjustLocationHipAnkle=True, hipOffsetX=0.036, hipOffsetZ=0.0175, ankleOffsetX=0.007
):
    ...
    for marker in markersInfo:
        ...
        if markersInfo[marker]["method"] == "jointCenter":
            ...
            if (
                ...
                or (marker == "RKnee" and markersInfo[marker]["parent"] == "sagittal_articulation_frame_r")
                or (marker == "LKnee" and markersInfo[marker]["parent"] == "sagittal_articulation_frame_l")
...
# %%
def runOpenPosePKTool(pathGenericSetupFile, pathModel, pathIKFile, timeRange, pathOutputFolder):
...
def exportTRCFromOpenPosePK(pathPKFolder, fileName):
    ...
    pathPKMarker = os.path.join(pathPKFolder, "PK_" + fileName + "_PointKinematics_" + markers[0] + "_pos.sto")
    ...
    for marker in markers[1:]:
        pathPKMarker = os.path.join(pathPKFolder, "PK_" + fileName + "_PointKinematics_" + marker + "_pos.sto")
...
def generateVisualizerJson(modelPath, ikPath, jsonOutputPath, statesInDegrees=True, vertical_offset=None):
    ...
    for iTime, time in enumerate(stateTime):
        ...
        for body in bodyset:
            ...
            visualizeDict["bodies"][body.getName()]["rotation"].append(
                body.getTransformInGround(state).R().convertRotationToBodyFixedXYZ().to_numpy().tolist()
            )
