...
smplx_path: str = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "../../model_data/smplx/"
)
model_path = os.getenv("SMPLX_PATH", smplx_path)

_default_config = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "mvmp1f_default.yml"
)


def mvmp_association_and_tracking(
    dataset, keypoints="body25", config_file=_default_config
):
    """
    ...
    Params:
        ...
        config_file (str) : path to mvmp1f config YAML.
...
def interpolate_points(points3d, method="cubic"):
    ...
    def interpolate_nan_col(col):
        ...
        f_interp = interpolate.interp1d(
            idx_good, col[idx_good], kind=method, bounds_error=False
        )
        col_interp = np.where(
            np.isfinite(col), col, f_interp(idx)
        )  # replace nans with interpolated values
        col_interp = np.where(
            np.isfinite(col_interp), col_interp, np.nanmean(col_interp)
...
def easymocap_fit_smpl_3d(
    ...
    body_model = load_model(
        model_path=model_path, model_type=body_model, skel_type=skel_type
    )
    ...
    if joints3d.shape[-1] == 3:
        ...
        joints3d = np.concatenate(
            [joints3d, np.ones((joints3d.shape[0], joints3d.shape[1], 1))], axis=-1
        )
...
def fit_multiple_smpl(results):
    ...
    for pid in ids:
        ...
        keypoints3d = np.stack(
            [k["keypoints3d"] for frame in results for k in frame if k["id"] == pid],
            axis=0,
        )
        frames = np.array(
            [i for i, frame in enumerate(results) for k in frame if k["id"] == pid]
        )

        body_params[pid] = smpl_from_keypoints3d(
            body_model,
            keypoints3d,
            config,
            args,
...
def get_joint_openpose(res, model_path=model_path, body_model="smpl"):
    ...
    return body_model(
        return_verts=False, return_joints=True, return_tensor=False, **res
    )
