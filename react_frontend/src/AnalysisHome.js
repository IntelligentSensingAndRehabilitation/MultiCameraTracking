...
const RecordingTable = ({ recordings, participant, session_date, callback }) => {
    ...
    const handleToggle = (recordingKey, isChecked) => {
        callback(participant, session_date, recordingKey, isChecked);
    };
...
const AnalysisHome = () => {
    ...
    const { fetchRecordingDb } = useContext(AcquisitionState);
    ...
    return (
        <div>
            ...
            <Container className="g-4 p-2" gap={3}>
                <ParticipantAccordion participants={recordingDb} callback={onToggleShouldProcess} />
            </Container>
            <Button onClick={resyncData}>Resync Data</Button>
        </div>
    );
};
